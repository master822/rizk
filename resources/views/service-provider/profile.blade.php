...
@section('title', 'الملف الشخصي - مقدم خدمات')
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="card-rizk text-center p-4">
                ...
                <p style="color: var(--text-muted);"><i class="fas fa-concierge-bell me-2"></i>{{ $service->service_name ?? 'لا توجد خدمات' }}</p>
                <a href="{{ route('service-provider.change-password') }}" class="btn btn-rizk-outline w-100 mt-2">تغيير كلمة المرور</a>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card-rizk p-4">
                ...
                <form action="{{ route('service-provider.profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="store_name" class="form-control form-rizk" value="{{ $service->service_name ?? '' }}">
                    </div>
                    <div class="mb-3">
                        <label style="color: var(--text-primary);">وصف الخدمة</label>
                        <textarea name="store_description" class="form-control form-rizk" rows="3">{{ $service->description ?? '' }}</textarea>
                    </div>
                    <div class="mb-3">
